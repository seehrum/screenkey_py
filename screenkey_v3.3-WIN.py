...
class ListenerThread(Thread):
    ...
    def on_press(self, key):
        ...
        try:
            ...
            # Detect if ALT GR is pressed (which Windows interprets as CTRL + ALT)
            if key == keyboard.Key.alt_gr:
                # Remove CTRL and ALT from active_modifiers if ALT GR is pressed
                self.active_modifiers.discard("CTRL")
                self.active_modifiers.discard("ALT")
                self.active_modifiers.add("ALT GR")
                self.update_method("ALT GR")
                return  # Skip further processing of ALT GR as CTRL + ALT

            # Handle individual modifier keys like CTRL, ALT, and SHIFT
            elif key in [keyboard.Key.ctrl_l, keyboard.Key.ctrl_r, keyboard.Key.alt_l, keyboard.Key.alt_r, keyboard.Key.shift, keyboard.Key.shift_r]:
                modifier_key = self.get_key_info(key)
                self.active_modifiers.add(modifier_key)
                # Update the display to show the active modifier key alone
                self.update_method(modifier_key)

            # Regular key press logic (e.g., SHIFT + A, CTRL + A)
            else:
                # Combine active modifier keys (if any) with the current key press
    ...
    def get_key_info(self, key):
        ...
        numpad_key_map = {
            ...
            50: '2', 51: '3', 52: '4', 53: '5',
            54: '6', 55: '7', 56: '8', 57: '9',
            48: '0', 3: 'PAUSE', 12: '5', 187: '+',
            189: '-', 186: 'Ç', 193: '/', 219: '´', 220: ']', 221: '[',
            222: '~', 191: ';', 188: ',', 190: '.',
        }
        ...
        try:
            if hasattr(key, 'vk'):
                if 65 <= key.vk <= 90:  # A to Z
                    return chr(key.vk)
            # Fix for vertical rectangle: ensure character is displayed instead of VK code
            return key.char.upper() if key.char is not None else key.name
